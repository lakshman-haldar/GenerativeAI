...
data_sources = {
    ...
    "Finance": "https://finance.yahoo.com/", #"https://www.bseindia.com/"],
...
nli_model = pipeline("text-classification", model="facebook/bart-large-mnli", device=0 if device == "cuda" else -1)

def calculate_truthfulness_score(response, reference_facts):
    """
    Computes a truthfulness score using a Natural Language Inference (NLI) model.
    
    Parameters:
    - response (str): The generated response from the LLM.
    - reference_facts (list): List of known true facts.

    Returns:
    - truthfulness_score (float): Score between 0 and 1.
    - details (list): List of classifications for each fact.
    """
    ...
    for fact in reference_facts:
        # Perform NLI classification using correct format
        result = nli_model([response, fact], return_all_scores=True)[0]

        # Extract label probabilities correctly
        label_map = {0: "ENTAILMENT", 1: "NEUTRAL", 2: "CONTRADICTION"}
        scores = {label_map[i]: entry["score"] for i, entry in enumerate(result)}

        # Determine the most probable classification
        label = max(scores, key=scores.get)
        confidence = scores[label]

        # Count entailment cases with confidence threshold
        if label == "ENTAILMENT" and confidence > 0.5:
            entailment_count += 1
    ...
    # Compute truthfulness score (ratio of entailments)
    truthfulness_score = entailment_count / total_facts if total_facts > 0 else 0.0

    return round(truthfulness_score, 2), results
...
tscore, details = calculate_truthfulness_score(response, reference_facts)

# Display results
print(f"Truthfulness Score: {tscore:.2f}") 
for item in details: 
    print(f"Fact: {item['fact']}, Prediction: {item['prediction']}, Confidence: {item['confidence']:.2f}") 
...
#-----------------------------------------------------------------------
#LLM-based F1 Score Calculation

# Simulated function for LLM response (replace with actual LLM model)
def get_llm_predictions(inputs):
    """
    Simulates LLM response for classification.
    Replace this with actual LLM API call.
    """
    label_mapping = {
        "Positive": ["great", "good", "excellent", "happy"],
        "Negative": ["bad", "worst", "poor", "sad"],
        "Neutral": ["okay", "fine", "normal", "average"]
    }
    
    predictions = []
    for label in inputs:
        # Assign the correct label or simulate an error
        if random.random() > 0.2:  # 80% chance to predict correctly
            predictions.append(label)
        else:
            predictions.append(random.choice(list(label_mapping.keys())))
...
# Dynamically get predictions from LLM
predicted_labels = get_llm_predictions(true_labels)

print("True Labels: ", true_labels)
print("Predicted Labels: ", predicted_labels, "\n")

# Try different averaging strategies
for avg in ['micro', 'macro', 'weighted']:
    precision, recall, f1_score, _ = precision_recall_fscore_support(true_labels, predicted_labels, average=avg)
    print(f"Averaging Method: {avg}")
    print(f"Precision: {precision:.2f}, Recall: {recall:.2f}, F1 Score: {f1_score:.2f}\n")

# Visualization - Precision, Recall, F1 Score
st.sidebar.header("Performance Metrics")
fig, ax = plt.subplots(figsize=(5, 3))
ax.bar(["Precison","Recall","F1-Score"], [precision,recall,f1_score,], color=['green', 'red','blue'])
ax.set_ylabel("Score")
ax.set_title("Performance Metrics")
st.sidebar.pyplot(fig)
#-------------------------------------------------------------------
# ROC Curve with LLM Fine-Tuned Model

# Load a fine-tuned LLM model for classification (Replace with your fine-tuned model)
llm_model = pipeline("text-classification", model="facebook/bart-large-mnli", return_all_scores=True)

# Example input texts for classification (Modify with real inputs)
texts = [
...
# Define class labels (Ensure these match the labels used in your fine-tuned model)
labels = ["Positive", "Negative", "Neutral"]

# Get predicted probabilities from the LLM
predicted_probs = []
for text in texts:
    output = llm_model(text)  # Model returns scores for all labels
    probs = {entry["label"]: entry["score"] for entry in output[0]}
    predicted_probs.append([probs.get(label, 0) for label in labels])  # Ensure probabilities align with label order

predicted_probs = np.array(predicted_probs)

# Example true labels (Modify based on ground truth data)
true_labels = np.array([1, 0, 2, 1, 2])  # Assuming 0 = Negative, 1 = Positive, 2 = Neutral

# Compute ROC curve for each class
rocfig, ax = plt.subplots(figsize=(8, 6))
for i, label in enumerate(labels):
    fpr, tpr, _ = roc_curve((true_labels == i).astype(int), predicted_probs[:, i])
    roc_auc = auc(fpr, tpr)
    ax.plot(fpr, tpr, lw=2, label=f"Class: {label} (AUC = {roc_auc:.2f})")
...
ax.set_title("ROC Curve for Fine-Tuned LLM Model")
ax.legend(loc="lower right")

# Display the ROC curve
st.sidebar.pyplot(rocfig)
...
# Display Performance Metrics
    ...
    ax_perp.bar(["Perplexity","Semantic similarity"], [perplexity,semantic_score], color=['orange','cyan'])
    ax_perp.set_ylabel("Score")
    ax_perp.set_title("Perplexity & Semantic Score")
    ...
    st.sidebar.write(f"Truthfulness Score: {tscore:.6f}")
...
# Visualization - Fact Overlap  & Contradiction, Entailment
    fig, ax = plt.subplots(figsize=(5, 3))
    ax.bar(["Contradiction","Truthfulness Score"], [contradiction_score,tscore], color=['orange','yellow'])
    ax.set_ylabel("Score")
    st.sidebar.pyplot(fig)
